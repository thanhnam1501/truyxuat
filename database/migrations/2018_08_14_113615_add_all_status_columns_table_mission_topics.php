...
class AddAllStatusColumnsTableMissionTopics extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
        Schema::table('mission_topics', function (Blueprint $table) {
            if (!Schema::hasColumn('mission_topics', 'is_submit_ele_copy')) {
                $table->tinyInteger('is_submit_ele_copy')->default(0)->comment('Nộp bản mềm - 0: chưa nộp, 1: đã nộp');
            }            
            if (!Schema::hasColumn('mission_topics', 'is_submit_hard_copy')) {

...
class NodeController extends Controller
{
 ...
 public function getFormCreate (Request $request){
  ...
  return view('user.node.AddNode',['node' => $data['node'], 'product_id' => $data['product_id'], 'user' => $user]);
...
Public function create(Request $request){
  ...
  return redirect()->route('user.product.edit',['id' => $data['product_id']]);
    ...
    {
      $data = Node::find($id);
      $company_id = Product::where('id', $data['product_id'])->first()->company_id;
      $user = Profile::where('company_id', $company_id)->get();
      // dd($user);

      return view('user.node.EditNode', ['data' => $data, 'user' => $user]);
    }
